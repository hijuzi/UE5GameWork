...
This reference defines the exact output format for all files generated by the `code-to-docs` skill. Every generated file must conform to the frontmatter schema and audience-level structure defined here. The Dataview queries in `output-structure.md` depend on these fields.
...
**Exemption list.** These are not "generated Markdown files" in the sense above, and Phase 3 verification must apply this same list or it fails on every run:

| Path | Why exempt |
|------|-----------|
| `Architecture/System Map.canvas` | JSON Canvas cannot carry Markdown frontmatter; adding it makes the file unopenable |
| `Documentation.base` | Bases files are YAML in their own schema; frontmatter makes them unopenable |
| `Index.md` | Carries only the five fields its template defines — the module-specific fields do not apply to a hub page |
| **`_state/**`** | Analysis artifacts, not reader-facing documentation. `_state/modules/<slug>.md` and `_state/synthesis.md` use their own frontmatter schema (`module`, `slug`, `roots`, `files`, `source-commit`, …), and `_state/analysis.json` is JSON. They are addressed by `<!-- c2d:sN -->` / `<!-- c2d:yN -->` markers, not by the fields below, and nothing links to them with a standard Markdown link |

Everything else under the vault root must carry the full block.
...
**`dependencies`**
List of module names this document's subject directly depends on. Only include direct dependencies — not transitive ones. Every listed name must correspond to an existing generated file.

**`related-notes`**
Relative paths to related docs that are conceptually linked but are not direct dependencies. Examples: a pattern that this module exemplifies, an onboarding doc that references this module, an architecture overview.
...
## 3. Link Conventions (Standard Markdown)

All cross-references between generated files use standard Markdown link syntax. This ensures the vault renders correctly in Obsidian, GitHub, VS Code, and any Markdown viewer.

| Use Case | Syntax | Notes |
|----------|--------|-------|
| Link to another doc (same dir) | `[display text](filename.md)` | Use `./` or `../` as needed for relative paths |
| Link to a specific section | `[display text](filename.md#section-name)` | Lowercase, hyphenate the heading text |
| Link to a sibling module | `[Module Name](../ModuleName/overview.md)` | Navigate up with `../` then into the target dir |

**Rules:**
- Every link MUST have a corresponding generated file. Do not create speculative links to files that have not been generated yet.
- **Use relative paths from the current file's location.** The path must resolve from where the file sits in the vault directory tree.
- **Link display text**: Use the target page's `title` frontmatter value, or a natural-language phrase when the title alone is unclear in context.
- **Source-code paths and filenames go in backticks, never as link targets.** A module report's prose names real source files; turning `output-structure.md` into a link invents a reference to a note that will never exist. Only the vault's own generated pages are link targets — module docs, architecture pages, health pages, patterns, onboarding, cross-cutting, and the index.
- When referencing a module in prose for the first time in a section, use a link. Subsequent mentions in the same section may use plain text.
- The `dependencies` frontmatter field stores plain module names (no brackets). The `related-notes` field stores relative paths.
...
**Module:** [{Module Name}](../modules/{Module Slug}/overview.md)
...
- Link each note to the corresponding entry in [Code Review](../health/Code Review.md) or [Limitations](../health/Limitations.md) for full context.
