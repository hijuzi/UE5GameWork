...
| **`_state/**`** | Analysis artifacts, not reader-facing documentation. `_state/modules/<slug>.md` and `_state/synthesis.md` use their own frontmatter schema (`module`, `slug`, `roots`, `files`, `source-commit`, …), and `_state/analysis.json` is JSON. They are addressed by `<!-- c2d:sN -->` / `<!-- c2d:yN -->` markers, not by the fields below, and nothing links to them with a `[[wikilink]]` |
