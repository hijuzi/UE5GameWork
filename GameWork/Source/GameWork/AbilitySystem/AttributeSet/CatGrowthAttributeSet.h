 ...
 *
 * 等级、经验、点数和五个加点属性放在一起：「经验发点」与「加点写属性」在同一集内完成，
 * 不需要约定两个集的初始化顺序；存档也能整集序列化，战斗属性不在这里，不会把残血存进去。
 *
...
public:
	...
	/** 元属性：经验入账。仅 GE execute 瞬间存在，结算后清零 */
	UPROPERTY(BlueprintReadOnly, Category = "Cat|Growth|Progression")
	...
	/** 活力：派生最大生命 */
	UPROPERTY(BlueprintReadOnly, Category = "Cat|Growth|Primary")
	...
	/** 防御（加点属性）：派生减伤率。与 `UCatUnitAttributeSet` 的 `DamageReduction` 是两层概念 */
	UPROPERTY(BlueprintReadOnly, Category = "Cat|Growth|Primary")
