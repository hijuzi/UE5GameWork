...
bool UCatCombatManagerSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
	return true;
}

void UCatCombatManagerSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);

	DataStore = NewObject<UCatCombatDataStore>(this, UCatCombatDataStore::StaticClass());
	DataStore->Initialize(this);

	HUDHandler = NewObject<UCatCombatHUDHandler>(this, UCatCombatHUDHandler::StaticClass());
	HUDHandler->Initialize(this);

	RoundTimer = NewObject<UCatCombatRoundTimer>(this, UCatCombatRoundTimer::StaticClass());
	RoundTimer->Initialize(this);
	RoundTimer->OnRoundTimeExpired.AddUObject(this, &UCatCombatManagerSubsystem::HandleRoundTimeExpired);

	TurnCoordinator = NewObject<UCatCombatTurnCoordinator>(this, UCatCombatTurnCoordinator::StaticClass());
	TurnCoordinator->Initialize(this, DataStore);

	// 监听战斗角色加入/离开事件，联动输入映射
	OnCombatCharacterChanged.AddDynamic(this, &UCatCombatManagerSubsystem::HandleCombatCharacterChanged);

	UE_LOG(LogCatCombatManager, Log, TEXT("CombatManagerSubsystem Initialized"));
}

void UCatCombatManagerSubsystem::Deinitialize()
{
	OnCombatCharacterChanged.RemoveDynamic(this, &UCatCombatManagerSubsystem::HandleCombatCharacterChanged);
...
void UCatCombatManagerSubsystem::StartBattle(FName CombatConfigRowName)
{
	if (DataStore)
	{
...
bool UCatCombatManagerSubsystem::EndBattle(bool bClearTeam)
{
	if (DataStore)
	{
...
void UCatCombatManagerSubsystem::AdvanceToNextRound()
{
	if (DataStore)
	{
...
void UCatCombatManagerSubsystem::AdvanceCombatTurn()
{
	if (!TurnCoordinator || !DataStore)
	{
...
ECombatResultType UCatCombatManagerSubsystem::GetCombatResult() const
{
	return DataStore ? DataStore->GetResult() : ECombatResultType::Unsettled;
}

void UCatCombatManagerSubsystem::SettleCombat()
{
...
TArray<ACharacter*> UCatCombatManagerSubsystem::GetCombatCharacterList(ECombatTeamType TeamType) const
{
	if (DataStore)
	{
...
void UCatCombatManagerSubsystem::TryExecuteCombatCharacterAbility(bool bJoined, ACharacter* Character)
{
	if (!Character)
	{
		return;
...
float UCatCombatManagerSubsystem::GetRoundProgress() const
{
	return RoundTimer ? RoundTimer->GetProgress() : 0.0f;
}

void UCatCombatManagerSubsystem::HandleCombatCharacterChanged(bool bJoined, ECombatTeamType TeamType, ACharacter* Character)
{
	// 尝试执行角色加入/离开战斗的 Ability
	TryExecuteCombatCharacterAbility(bJoined, Character);
}

void UCatCombatManagerSubsystem::HandleRoundTimeExpired()
{
...
void UCatCombatManagerSubsystem::RegisterScenePoint(ACatCombatScenePoint* ScenePoint)
{
	if (!ScenePoint)
	{
		return;
	...
	if (const TWeakObjectPtr<ACatCombatScenePoint>* Existing = ScenePointMap.Find(RowName))
	{
...
void UCatCombatManagerSubsystem::UnregisterScenePoint(ACatCombatScenePoint* ScenePoint)
{
	if (!ScenePoint)
	{
		return;
...
ACatCombatScenePoint* UCatCombatManagerSubsystem::GetScenePoint(FName RowName) const
{
	if (const TWeakObjectPtr<ACatCombatScenePoint>* Found = ScenePointMap.Find(RowName))
	{
		return Found->Get();
	}
	return nullptr;
