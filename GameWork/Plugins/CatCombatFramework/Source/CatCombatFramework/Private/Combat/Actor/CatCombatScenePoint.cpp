...
#include "Combat/DataAsset/CatCombatCameraDataAsset.h"
...
ACatCombatScenePoint::ACatCombatScenePoint()
{
...
void ACatCombatScenePoint::OnConstruction(const FTransform& Transform)
{
...
void ACatCombatScenePoint::BeginPlay()
{
	Super::BeginPlay();

	if (UGameInstance* GameInstance = GetGameInstance())
	{
		if (UCatCombatManagerSubsystem* CombatManager = GameInstance->GetSubsystem<UCatCombatManagerSubsystem>())
		{
...
void ACatCombatScenePoint::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if (UGameInstance* GameInstance = GetGameInstance())
	{
		if (UCatCombatManagerSubsystem* CombatManager = GameInstance->GetSubsystem<UCatCombatManagerSubsystem>())
...
void ACatCombatScenePoint::Destroyed()
{
#if WITH_EDITOR
...
#if WITH_EDITOR
void ACatCombatScenePoint::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(ACatCombatScenePoint, CombatConfigRowName) ||
		MemberPropertyName == GET_MEMBER_NAME_CHECKED(ACatCombatScenePoint, CombatConfigRowName) ||
		PropertyName == GET_MEMBER_NAME_CHECKED(ACatCombatScenePoint, bShowCharacterPreview) ||
		MemberPropertyName == GET_MEMBER_NAME_CHECKED(ACatCombatScenePoint, bShowCharacterPreview))
	{
		if (const UWorld* World = GetWorld())
		{
			if (!World->IsGameWorld())
...
void ACatCombatScenePoint::UpdateCombatData()
{
#if WITH_EDITOR
	// 仅在编辑器未播放时刷新
	if (const UWorld* World = GetWorld())
...
#if WITH_EDITOR
void ACatCombatScenePoint::RebuildPreviewActors()
{
	...
	if (const FCatCombatDataTableRow* Row = GetCombatConfigRow())
	{
...
void ACatCombatScenePoint::ClearPreviewActors(TArray<TObjectPtr<AActor>>& PreviewActors)
{
	ClearPreviewActorsImpl(PreviewActors);
}

void ACatCombatScenePoint::BuildPreviewActors(ECombatTeamType TeamType, const TArray<TObjectPtr<UCatCombatCharacterDataAsset>>& Teams, TArray<TObjectPtr<AActor>>& OutPreviewActors)
{
	// 关闭预览角色开关时不生成任何角色
...
#if WITH_EDITOR
void ACatCombatScenePoint::BuildCameraPreview(const UCatCombatCameraDataAsset* CameraData)
{
	// 无相机配置或没有有效世界时不再生成
	UWorld* World = GetWorld();
	if (!CameraData || !CameraData->CameraClass || !World)
	{
		UE_LOG(LogCatCombatScenePoint, Warning, TEXT("ACatCombatScenePoint::BuildCameraPreview: 相机未生成（数据表行 %s，CameraData=%s，CameraClass=%s）"),
	...
	if (!CameraActor)
	{
		UE_LOG(LogCatCombatScenePoint, Warning, TEXT("ACatCombatScenePoint::BuildCameraPreview: 相机生成失败（类 %s）"), *CameraData->CameraClass->GetName());
		return;
	}
	if (!CameraActor->GetCameraComponent())
	{
		UE_LOG(LogCatCombatScenePoint, Warning, TEXT("ACatCombatScenePoint::BuildCameraPreview: 相机 %s 缺少 CameraComponent，已销毁"), *CameraActor->GetName());
...
void ACatCombatScenePoint::ClearCameraPreview()
{
	ClearPreviewActorsImpl(SceneCameraActors);
}

void ACatCombatScenePoint::RebuildSceneCamera()
{
	// 清理旧的相机 Actor（若已保存进关卡，需保存关卡后才会从磁盘移除）
	ClearCameraPreview();

	// 未配置有效数据表行时不再生成
	const FCatCombatDataTableRow* Row = GetCombatConfigRow();
	if (!Row)
	{
		UE_LOG(LogCatCombatScenePoint, Warning, TEXT("ACatCombatScenePoint::RebuildSceneCamera: 数据表行 %s 不存在或未配置，无法生成相机"), *CombatConfigRowName.ToString());
		return;
	}
...
TArray<FTransform> ACatCombatScenePoint::GetSpawnTransforms(ECombatTeamType TeamType) const
{
	...
	const TArray<TObjectPtr<UCatCombatCharacterDataAsset>>* Teams = nullptr;
	switch (TeamType)
	{
	...
	for (const TObjectPtr<UCatCombatCharacterDataAsset>& TeamAsset : *Teams)
	{
		if (!TeamAsset)
		{
			continue;
...
TArray<FTransform> ACatCombatScenePoint::GetPlayerSpawnTransforms() const
{
	return GetSpawnTransforms(ECombatTeamType::Player);
}

TArray<FTransform> ACatCombatScenePoint::GetEnemySpawnTransforms() const
{
	return GetSpawnTransforms(ECombatTeamType::Enemy);
}

FName ACatCombatScenePoint::GetCombatConfigRowName() const
{
	return CombatConfigRowName;
}

ACameraActor* ACatCombatScenePoint::GetMainCameraActor() const
{
...
TArray<FName> ACatCombatScenePoint::GetCombatConfigRowNames()
{
	TArray<FName> RowNames;
	if (const UCatCombatSettings* Settings = GetDefault<UCatCombatSettings>())
	{
...
const FCatCombatDataTableRow* ACatCombatScenePoint::GetCombatConfigRow() const
{
	if (CombatConfigRowName.IsNone())
	{
		return nullptr;
	}
	const UCatCombatSettings* Settings = GetDefault<UCatCombatSettings>();
	if (!Settings)
	{
		return nullptr;
	}
	...
	if (Table)
	{
		return Table->FindRow<FCatCombatDataTableRow>(CombatConfigRowName, TEXT("ACatCombatScenePoint::GetCombatConfigRow"));
	}
	return nullptr;
}
