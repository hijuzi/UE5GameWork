...
UCatCharacterTurnComponent::UCatCharacterTurnComponent()
{
	PrimaryComponentTick.bCanEverTick = false;
}

UCatCharacterTurnComponent* UCatCharacterTurnComponent::GetCatCharacterTurnComponent(const AActor* Actor)
{
	return Actor ? Actor->FindComponentByClass<UCatCharacterTurnComponent>() : nullptr;
}

void UCatCharacterTurnComponent::SetCoordinator(UCatCombatTurnCoordinator* InCoordinator)
{
	Coordinator = InCoordinator;
}

void UCatCharacterTurnComponent::RegisterCombatAbility(const FCatTurnActionData& TurnActionData, const FGameplayTag& Tag, const FGameplayAbilitySpecHandle& Handle)
{
	if (!Tag.IsValid())
	{
		return;
	...
	const auto RegisterTo = [this, &Tag, &Handle, &TurnActionData](ECombatTurnRole Role)
	{
		...
		TMap<FGameplayTag, FCatTurnActionData>& DataMap = (Role == ECombatTurnRole::Defender) ? DefenderAbilityData : AttackerAbilityData;
	...
	{
	case ECatTurnActionType::Attacker:
		RegisterTo(ECombatTurnRole::Attacker);
		break;
	case ECatTurnActionType::Defender:
		RegisterTo(ECombatTurnRole::Defender);
		break;
	case ECatTurnActionType::Both:
		RegisterTo(ECombatTurnRole::Attacker);
		RegisterTo(ECombatTurnRole::Defender);
		break;
	default:
...
void UCatCharacterTurnComponent::UnregisterCombatAbility(const FCatTurnActionData& TurnActionData, const FGameplayAbilitySpecHandle& Handle)
{
...
void UCatCharacterTurnComponent::SetState(ECharacterTurnState NewState)
{
	if (State == NewState)
	{
		return;
...
FGameplayTag UCatCharacterTurnComponent::GetStateTag(ECharacterTurnState InState)
{
	switch (InState)
	{
...
UAbilitySystemComponent* UCatCharacterTurnComponent::GetCachedASC() const
{
...
bool UCatCharacterTurnComponent::CanActivateAbilityByTag(const FGameplayTag& Tag, ECombatTurnRole Role) const
{
	if (!Tag.IsValid())
	{
		return false;
...
TMap<FGameplayTag, int32> UCatCharacterTurnComponent::GetAllActivationCounts() const
{
...
const FCatTurnActionData* UCatCharacterTurnComponent::FindAbilityData(const FGameplayTag& Tag, ECombatTurnRole Role) const
{
	const TMap<FGameplayTag, FCatTurnActionData>& DataMap = (Role == ECombatTurnRole::Defender) ? DefenderAbilityData : AttackerAbilityData;
	return DataMap.Find(Tag);
}

const TMap<FGameplayTag, int32>* UCatCharacterTurnComponent::GetCountMapForTag(const FGameplayTag& Tag, ECombatTurnRole Role) const
{
	const FCatTurnActionData* Data = FindAbilityData(Tag, Role);
	if (!Data || Data->LimitType == ECatActivationLimitType::IgnoreLimit)
	{
		return nullptr;  // 未注册 / IgnoreLimit（忽略限制）：不记录
	}
	if (Data->LimitType == ECatActivationLimitType::Limited)
	{
		return (Data->LimitScope == ECatActivationLimitScope::PerTurn) ? &PerTurnActivationCounts : &BattleActivationCounts;
	}
	return &UnlimitedActivationCounts;  // Unlimited（无限次数）
}

TMap<FGameplayTag, int32>* UCatCharacterTurnComponent::GetCountMapForTag(const FGameplayTag& Tag, ECombatTurnRole Role)
{
	const FCatTurnActionData* Data = FindAbilityData(Tag, Role);
	if (!Data || Data->LimitType == ECatActivationLimitType::IgnoreLimit)
	{
...
int32 UCatCharacterTurnComponent::GetActivationCount(const FGameplayTag& Tag, ECombatTurnRole Role) const
{
...
void UCatCharacterTurnComponent::IncrementActivationCount(const FGameplayTag& Tag, ECombatTurnRole Role)
{
	if (!Tag.IsValid())
	{
		return;
...
void UCatCharacterTurnComponent::ResetActivationCount(const FGameplayTag& Tag)
{
...
void UCatCharacterTurnComponent::ResetAllActivationCounts()
{
...
void UCatCharacterTurnComponent::ResetPerTurnActivationCounts()
{
	// PerTurn 分类已独立成 map，直接清空即可，无需遍历判断
	PerTurnActivationCounts.Empty();
}

bool UCatCharacterTurnComponent::GetRemainingActivationCount(const FGameplayTag& Tag, ECombatTurnRole Role, int32& OutRemainingCount) const
{
	// 直接从注册时缓存的 TurnActionData 读取限制配置（不再依赖能力实例/CDO）
	const FCatTurnActionData* Data = FindAbilityData(Tag, Role);
	if (!Data || Data->LimitType != ECatActivationLimitType::Limited)
	{
		return false;
	}
...
UClass* UCatCharacterTurnComponent::GetCurrentActingAbilityClass() const
{
...
void UCatCharacterTurnComponent::ApplyStateTagToASC()
{
...
bool UCatCharacterTurnComponent::ShouldEnforceActionSlotLimit() const
{
...
FGameplayTagContainer UCatCharacterTurnComponent::GetRemainingActionTags(bool bAllowRepeats) const
{
	...
	for (const FGameplayTag& Tag : RoleTags)
	{
		...
		const FCatTurnActionData* Data = FindAbilityData(Tag, TurnRole);
		if (!Data || Data->LimitType != ECatActivationLimitType::Limited)
		{
			continue;
		}
	...
	for (const FGameplayTag& Tag : RoleTags)
	{
		...
		const bool bCanInitiate = (ActivatedCount == 0)
			|| (bAllowRepeats && (Data->LimitType == ECatActivationLimitType::Unlimited || Data->LimitCount - ActivatedCount > 0));
		if (bCanInitiate)
		{
...
void UCatCharacterTurnComponent::OnEnterDecisionMaking()
{
...
void UCatCharacterTurnComponent::OnEnterActed()
{
	// 结束上报协调器（从 PendingReadySet 移除角色）
	if (UCatCombatTurnCoordinator* Coord = Coordinator.Get())
	{
		Coord->NotifyTurnFinished(GetOwner<ACharacter>());
	}
}

void UCatCharacterTurnComponent::OnEnterDeferred()
{
...
void UCatCharacterTurnComponent::HandleBeginTurn(ECombatTurnRole InRole)
{
	TurnRole = InRole;
	...
	// 保持协调器不感知组件类型（延续 ICatCombatCoreInterface 接口解耦）。
	if (!Coordinator.IsValid())
	{
		if (const UGameInstance* GI = GetOwner()->GetGameInstance())
		{
			if (UCatCombatManagerSubsystem* Subsystem = GI->GetSubsystem<UCatCombatManagerSubsystem>())
			{
...
TArray<FGameplayTag> UCatCharacterTurnComponent::GetRoleTags() const
{
...
TArray<FGameplayTag> UCatCharacterTurnComponent::GetAttackerTags() const
{
	TArray<FGameplayTag> Tags;
	AttackerAbilityHandles.GetKeys(Tags);
	return Tags;
...
TArray<FGameplayTag> UCatCharacterTurnComponent::GetDefenderTags() const
{
	TArray<FGameplayTag> Tags;
	DefenderAbilityHandles.GetKeys(Tags);
	return Tags;
...
UGameplayAbility* UCatCharacterTurnComponent::GetAbilityInstanceByTag(const FGameplayTag& Tag, ECombatTurnRole Role) const
{
...
bool UCatCharacterTurnComponent::TryActivateActionByTag(const FGameplayTag& Tag, ECombatTurnRole Role)
{
	if (!Tag.IsValid())
	{
		return false;
...
FGameplayTag UCatCharacterTurnComponent::FindRoleTag(const FGameplayTagContainer& AssetTags) const
{
...
void UCatCharacterTurnComponent::NotifyActionStarted(const FGameplayTagContainer& AssetTags)
{
...
bool UCatCharacterTurnComponent::IsCoordinatorFinishing() const
{
	const UCatCombatTurnCoordinator* Coord = Coordinator.Get();
	return Coord && Coord->GetPhase() == ECombatCampPhase::Finishing;
}

void UCatCharacterTurnComponent::NotifyActionFinished(const FGameplayTagContainer& AssetTags)
{
...
void UCatCharacterTurnComponent::ScheduleTurnFinished()
{
...
void UCatCharacterTurnComponent::NotifyTurnFinished()
{
	// 上报协调器由 OnEnterActed 状态迁移钩子统一处理
	SetState(ECharacterTurnState::Acted);
}

void UCatCharacterTurnComponent::NotifyDeferred()
{
	// 上报协调器由 OnEnterDeferred 状态迁移钩子统一处理
	SetState(ECharacterTurnState::Deferred);
}

void UCatCharacterTurnComponent::TryReactiveDefense(const FReactiveDefenseRequest& Request, FReactiveDefenseResult& Out)
{
	// 抽象扩展点：具体防御行为（闪避/格挡等）通过「防御类型 → 处理节点」注册扩展，第 5 步接入
	Out.bSucceeded = false;
}

void UCatCharacterTurnComponent::NotifyHitResolve(const FHitResolveContext& Context)
{
