...
#include "CatCharacterTurnComponent.generated.h"

class UCatCombatTurnCoordinator;
 ...
 * 由角色类的 ICatCombatCoreInterface::OnBeginTurn_Implementation 转发调用 HandleBeginTurn。
 *
 * 战斗能力采用「注册制」：授予方在给 ASC 授予能力时通过 RegisterCombatAbility 显式注册
 * （携带 FCatTurnActionData 限次配置），组件据此维护 Tag → Handle 映射与激活限次计数
 * （PerTurn / WholeBattle / Unlimited），供决策层查询剩余可行动能力。
 */
UCLASS(ClassGroup = (Combat), meta = (BlueprintSpawnableComponent))
class CATCOMBATFRAMEWORK_API UCatCharacterTurnComponent : public UActorComponent
{
	GENERATED_BODY()

public:
	UCatCharacterTurnComponent();
	...
	static UCatCharacterTurnComponent* GetCatCharacterTurnComponent(const AActor* Actor);
	...
	void SetCoordinator(UCatCombatTurnCoordinator* InCoordinator);

	/** 注册单个战斗能力：根据 TurnActionData 将能力的主标识 Tag 与 SpecHandle 加入对应职责映射（由能力授予时自行调用） */
	void RegisterCombatAbility(const FCatTurnActionData& TurnActionData, const FGameplayTag& Tag, const FGameplayAbilitySpecHandle& Handle);

	/** 注销单个战斗能力：根据 TurnActionData 从对应职责映射移除该 Handle 的所有 Tag 条目（能力移除时调用） */
	void UnregisterCombatAbility(const FCatTurnActionData& TurnActionData, const FGameplayAbilitySpecHandle& Handle);
	...
	const TMap<FGameplayTag, FCatTurnActionData>& GetAttackerAbilityData() const { return AttackerAbilityData; }

	/** 获取防守方能力 TurnActionData 缓存（Tag → 配置，只读，供调试分类/查询） */
	const TMap<FGameplayTag, FCatTurnActionData>& GetDefenderAbilityData() const { return DefenderAbilityData; }
...
protected:
	...
	/** 所属协调器（弱引用，避免强引用环） */
	UPROPERTY(Transient)
	TWeakObjectPtr<UCatCombatTurnCoordinator> Coordinator;
...
private:
	...
	const FCatTurnActionData* FindAbilityData(const FGameplayTag& Tag, ECombatTurnRole Role) const;
	...
	UPROPERTY(Transient)
	TMap<FGameplayTag, FCatTurnActionData> AttackerAbilityData;

	/** 防守方能力 TurnActionData 缓存（Tag → TurnActionData，注册时缓存，供限制次数判定） */
	UPROPERTY(Transient)
	TMap<FGameplayTag, FCatTurnActionData> DefenderAbilityData;
};
