...
#include "CatCombatFunctionLibrary.generated.h"
 ...
 */
UCLASS()
class CATCOMBATFRAMEWORK_API UCatCombatFunctionLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	/** 供蓝图节点参数下拉框使用的选项：直接复用 ACatCombatScenePoint 的静态实现 */
	...
	static void SetBattleHUDVisible(const UObject* WorldContextObject, bool bVisible, const FCatCombatHUDVisibilityParams& Params = FCatCombatHUDVisibilityParams());
	...
	static const FCatCombatDataTableRow* GetCombatConfigRowFromWorld(const UObject* WorldContextObject, FName CombatConfigRowName);

private:
	static class UCatCombatManagerSubsystem* GetCombatManagerSubsystem(const UObject* WorldContextObject);
	...
	static void CollectSkillCandidates(const TArray<FAISkillWeight>& Source, ACharacter* Character,
		class UCatCharacterTurnComponent* TurnComp, ECombatTurnRole Role, TArray<FAISkillWeight>& OutCandidates);
