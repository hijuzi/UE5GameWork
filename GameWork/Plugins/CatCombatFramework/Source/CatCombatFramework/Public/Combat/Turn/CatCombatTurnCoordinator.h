...
#include "CatCombatTurnCoordinator.generated.h"
 ...
 * 不感知角色个体阶段（角色阶段由 UCatCharacterTurnComponent 自驱动维护）。
 *
 * 由 UCatCombatManagerSubsystem 持有（弱引用记录持有者，避免强引用环）。
 */
UCLASS()
class CATCOMBATFRAMEWORK_API UCatCombatTurnCoordinator : public UObject
{
	GENERATED_BODY()

public:
	/** 初始化：记录持有者与数据层（弱引用） */
	void Initialize(UCatCombatManagerSubsystem* InOwner, UCatCombatDataStore* InDataStore);
...
private:
	...
	/** 数据层（弱引用，由持有者拥有） */
	UPROPERTY(Transient)
	TWeakObjectPtr<UCatCombatDataStore> DataStore;
