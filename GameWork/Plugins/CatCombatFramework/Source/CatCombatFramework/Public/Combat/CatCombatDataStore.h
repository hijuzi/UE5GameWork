...
#include "CatCombatDataStore.generated.h"
 ...
 */
UCLASS()
class CATCOMBATFRAMEWORK_API UCatCombatDataStore : public UObject
{
	GENERATED_BODY()

public:
	/** 初始化：记录持有者（弱引用） */
	void Initialize(UCatCombatManagerSubsystem* InOwner);
	...
	ACatCombatScenePoint* GetScenePoint() const;
...
private:
	...
	int32 SpawnTeamCharacters(UWorld* World, ACatCombatScenePoint* ScenePoint, ECombatTeamType TeamType, const TArray<TObjectPtr<UCatCombatCharacterDataAsset>>& Teams);

	/** 持有者（弱引用，避免强引用环） */
	UPROPERTY(Transient)
	TWeakObjectPtr<UCatCombatManagerSubsystem> OwnerSubsystem;

	/** 当前战斗关联的场景点（弱引用，不拥有、不阻止 GC；场景点被销毁后自动失效） */
	UPROPERTY(Transient)
	TWeakObjectPtr<ACatCombatScenePoint> CurrentScenePoint;
