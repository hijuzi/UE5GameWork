...
#include "CatCombatTypes.generated.h"
...
/**
 * 阵营回合阶段（全局主状态机 UCatCombatTurnCoordinator 的状态）。
...
/**
 * 角色自身回合阶段（角色级状态机 UCatCharacterTurnComponent 的状态）。
 */
UENUM(BlueprintType)
...
UENUM(BlueprintType)
enum class ECatTurnActionType : uint8
{
...
/** 可激活次数限制类型 */
UENUM(BlueprintType)
enum class ECatActivationLimitType : uint8
{
...
/** 有限次数生效时机 */
UENUM(BlueprintType)
enum class ECatActivationLimitScope : uint8
{
...
struct FCatTurnActionData
{
	GENERATED_BODY()

	/** TurnAction 类型 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "TurnAction")
	ECatTurnActionType TurnActionType = ECatTurnActionType::Attacker;

	/** 可激活次数限制类型 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "TurnAction")
	ECatActivationLimitType LimitType = ECatActivationLimitType::Unlimited;

	/** 有限次数生效时机（仅当 LimitType 为「限制次数」时显示） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "TurnAction", meta = (EditCondition = "LimitType == ECatActivationLimitType::Limited", EditConditionHides))
	ECatActivationLimitScope LimitScope = ECatActivationLimitScope::PerTurn;
