...
#include "CatCombatScenePoint.generated.h"

class UBillboardComponent;
class UDataTable;
class USceneComponent;
class UWorld;
class UCatCombatCharacterDataAsset;
class UCatCombatCameraDataAsset;
class UCatCombatManagerSubsystem;
class ACameraActor;
struct FCatCombatDataTableRow;
struct FPropertyChangedEvent;

/**
 * 战斗场景点：回合战斗场景的配置容器 + 编辑器站位预览。
 *
 * - 作为配置容器，通过 CombatConfigRowName 关联战斗数据表行（FCatCombatDataTableRow），
 ...
 */
UCLASS()
class CATCOMBATFRAMEWORK_API ACatCombatScenePoint : public AActor
{
	GENERATED_BODY()

public:
	ACatCombatScenePoint();
	...
	const FCatCombatDataTableRow* GetCombatConfigRow() const;
	...
	/** 供编辑器下拉框使用的选项：返回 UCatCombatSettings::CombatConfigTable 的全部行名 */
	UFUNCTION()
...
private:
	...
	// ---- 战斗配置（数据表） ----
	/** 关联的战斗配置数据表行名（选项仅来自 UCatCombatSettings::CombatConfigTable 的行名，编辑器中渲染为单选下拉框）。
	...
	void BuildPreviewActors(ECombatTeamType TeamType, const TArray<TObjectPtr<UCatCombatCharacterDataAsset>>& Teams, TArray<TObjectPtr<AActor>>& OutPreviewActors);

	/** 按相机资产生成场景相机 Actor（无相机配置或没有有效世界时不生成） */
	void BuildCameraPreview(const UCatCombatCameraDataAsset* CameraData);
