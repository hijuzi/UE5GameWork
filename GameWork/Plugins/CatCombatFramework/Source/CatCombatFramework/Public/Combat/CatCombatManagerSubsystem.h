...
#include "CatCombatManagerSubsystem.generated.h"

class ACharacter;
class ACatCombatScenePoint;
class UCatCombatDataStore;
class UCatCombatHUDHandler;
class UCatCombatRoundTimer;
 ...
 * 数据状态（角色映射、结算结果）由数据层 UCatCombatDataStore 维护。
 */
UCLASS(BlueprintType, Blueprintable)
class CATCOMBATFRAMEWORK_API UCatCombatManagerSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
...
public:
	...
	/** 获取战斗数据层 */
	UCatCombatDataStore* GetDataStore() const { return DataStore; }

	/** 获取战斗 HUD 处理器（执行层） */
	UCatCombatHUDHandler* GetHUDHandler() const { return HUDHandler; }

	/** 获取回合计时器（执行层） */
	UCatCombatRoundTimer* GetRoundTimer() const { return RoundTimer; }

	/** 获取回合协调器（决策层，全局主状态机） */
	UCatCombatTurnCoordinator* GetTurnCoordinator() const { return TurnCoordinator; }
	...
	void RegisterScenePoint(ACatCombatScenePoint* ScenePoint);

	/** 注销场景点（按配置行名移除，附指针校验避免误删） */
	void UnregisterScenePoint(ACatCombatScenePoint* ScenePoint);

	/** 按配置行名获取已注册场景点（未注册或已销毁返回 nullptr） */
	ACatCombatScenePoint* GetScenePoint(FName RowName) const;

protected:
	/** 战斗数据层（纯数据 CRUD） */
	UPROPERTY(Transient)
	TObjectPtr<UCatCombatDataStore> DataStore;

	/** 战斗 HUD 处理器（执行层） */
	UPROPERTY(Transient)
	TObjectPtr<UCatCombatHUDHandler> HUDHandler;

	/** 回合计时器（执行层） */
	UPROPERTY(Transient)
	TObjectPtr<UCatCombatRoundTimer> RoundTimer;

	/** 回合协调器（决策层，全局主状态机） */
	UPROPERTY(Transient)
	TObjectPtr<UCatCombatTurnCoordinator> TurnCoordinator;
...
private:
	...
	TMap<FName, TWeakObjectPtr<ACatCombatScenePoint>> ScenePointMap;
};
