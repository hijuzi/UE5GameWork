...
void FGASAbilitieNode::CreateChild()
{
	...
	for (UGameplayAbility* Instance : Instances)
	{
		...
		TArray<TObjectPtr<UGameplayTask>> ActiveTasks = *ActiveTasksPtr->ContainerPtrToValuePtr<TArray<TObjectPtr<UGameplayTask>>>(Instance);

		for (TObjectPtr<UGameplayTask>& Item : ActiveTasks)
		{
			if(!Item) continue;
