...
> | **编译适配** | `GAS_MOD_13`（另含并入 `GAS_MOD_09` 的委托执行） | 插件形式下的 `Internal` include 路径与委托执行适配 |
...
4. 提供 `bTurnBased` 开关，支持运行时在即时制 ↔ 回合制之间切换（切换时已在计时容器内的 Timer 需迁移或清理，见第 11 章）；
...
| `GAS_MOD_09` | `AbilityTimerManager.h/.cpp` | **新增文件**（时间轴 Timer 管理器，整体用 `START/END` 包裹；阶段二的多轴扩展、插件形式下的**委托执行适配**均并入本编号） |

> 说明：改造中还包含 include 语句的标记（`GAS_MOD_07` / `GAS_MOD_12`）。

### 4.1 新增 `FAbilityTimerManager`（`GAS_MOD_09` 初版形态）

> 下方为阶段一的**初版单刻度形态**（历史留档）；阶段二已升级为多轴版（见 5.5）。
> 注意：源码中**已不存在** `TickTurn` / `GetAbilityCurrentTurn`，容器字段 `Turn` 也已改为 `FAbilityTimerAxis::Counter`。
...
| 子标记 | 位置 | 改动 |
|--------|------|------|
| `07a` | `ApplyGameplayEffectSpec` 的 Duration Timer 注册 | 回合制改用 `SetAbilityTimer`（Duration=刻度）；兜底分支回合制下改为 1 个刻度后触发 |
| `07b` | 同函数的 Period Timer 注册 | 回合制改用 `SetAbilityTimer`（Period=刻度）；"应用即触发"改为直接 `Delegate.ExecuteIfBound()` |
...
> 以下为 `Private/AbilityTimerManager.cpp` 的**实际实现**，相对朴素遍历做了三处稳健性处理：
> ① 每个步进内**重新快照命中的轴 Tag**（避免回调修改容器导致迭代器失效）；
> ② 轴的句柄数组**按值拷贝**后再遍历（回调移除 GE 不会破坏遍历）；
> ③ 执行前**快照 `bLoop` / `Rate`**、循环顺延前重新 `FindTimer`（避免回调清除 Timer 后访问失效的 `FTimerData`）。
...
**`SetAbilityTimer` 的两处语义兜底**（同文件）：

- `InTiming` 无效 → 输出 `UE_LOG` 警告并**放弃注册**；
- `InTiming == TimeAxis.Round`（命名空间标签）→ 输出 `UE_LOG` 警告（仅提示，不阻止）。
...
| `GAS_MOD_09`（扩展） | `AbilityTimerManager.h/.cpp` | 多轴化（`FAbilityTimerAxis` / `TickTimeline`），移除 `TickTurn`；插件形式下的委托执行适配 |
...
| delegate 执行 | `FTimerUnifiedDelegate::Execute()` **未导出**（无 `ENGINE_API`，引擎内同 DLL 可直呼），插件跨 DLL 会 LNK2019；改为取内部 `VariantDelegate.TryGet<FTimerDelegate>()` 执行（改动位于 `AbilityTimerManager.cpp`，**并入 `GAS_MOD_09`**，见 5.6） |
...
11. ✅ 插件形式编译适配：`Internal` include 路径（两个 `*.Build.cs`）—— `GAS_MOD_13`；委托执行（`AbilityTimerManager.cpp`）—— 并入 `GAS_MOD_09`。
...
> 期间解决了 2 处"插件形式"特有的阻塞（详见 `源码修改记录.md`）：
> 1. **`GAS_MOD_13`**：`Internal` 目录头不可见（`CoreUObject/Internal`、`GameplayTagsEditor/Internal`）
>    → 两个 `*.Build.cs` 显式补充 `PrivateIncludePaths`（C++ 源码保持与引擎原版逐字一致，便于后续引擎升级回刷合并）；
> 2. **并入 `GAS_MOD_09`**：`FTimerUnifiedDelegate::Execute()` 未导出（无 `ENGINE_API`）
>    → `AbilityTimerManager.cpp` 中改用 `VariantDelegate.TryGet<FTimerDelegate>()` 执行。
>
