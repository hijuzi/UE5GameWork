...
class UAbilitySystemComponent : public UGameplayTasksComponent, public IGameplayTagAssetInterface, public IAbilitySystemReplicationProxyInterface
{
	...
	/** Finds existing AttributeSet for the given actor */
	template <class T >
	static const T* GetSetOnActor(AActor* InActor)
	{
	...
	*/
	 ...
	 */
	UE_API virtual FActiveGameplayEffectHandle SetActiveGameplayEffectInhibit(FActiveGameplayEffectHandle&& ActiveGEHandle, bool bInhibit, bool bInvokePredictedEffects);
...
public:
	...
	/** Returns the next section name for the current montage. */
	UE_API FName CurrentMontageGetNextSectionName() const;
...
private:
	...
	void RegisterAbilityTriggers(const FGameplayAbilitySpec& Spec, TConstArrayView<FAbilityTriggerData> AbilityTriggers);
	void UnregisterAbilityTriggers(const FGameplayAbilitySpec& Spec, TConstArrayView<FAbilityTriggerData> AbilityTriggers);
...
protected:
	/** Called before activation for LocalPredicted Ability to execute CharacterMovementComponent->FlushServerMoves 
	 * When overriding make sure to include Super::PrePredictionActivation(Ability) */
	UE_API virtual void PrePredictionActivation(const UGameplayAbility* Ability);
