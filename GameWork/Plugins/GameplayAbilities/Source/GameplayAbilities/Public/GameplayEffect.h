...
/** Describes what happens to a Gameplay Effect that was applied on-application by another Gameplay Effect
 *  (via UAdditionalEffectsGameplayEffectComponent) when the applying effect is removed. */
UENUM()
enum class EGameplayEffectGrantedEffectRemovalPolicy : uint8
{
	/** The applied Gameplay Effect manages its own lifetime independently. This is the default behavior. */
	GrantedEffectControlsOwnLifetime,

	/** The applied Gameplay Effect will be removed when the Gameplay Effect that applied it is removed.
	 *  Cannot be used when the granted effect has an Instant duration (instant GEs are never persisted). */
...
struct FConditionalGameplayEffect
{
	...
	/** What happens to this applied effect when the Gameplay Effect that applied it is removed.
	 *  Only relevant when this entry is used in UAdditionalEffectsGameplayEffectComponent::OnApplicationGameplayEffects. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = GameplayEffect)
	EGameplayEffectGrantedEffectRemovalPolicy RemovalPolicy = EGameplayEffectGrantedEffectRemovalPolicy::GrantedEffectControlsOwnLifetime;

	/** Number of stacks to remove when RemovalPolicy is RemoveGrantedEffectOnEnd.
	 *  INDEX_NONE (default) removes all stacks. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = GameplayEffect, meta = (EditCondition = "RemovalPolicy == EGameplayEffectGrantedEffectRemovalPolicy::RemoveGrantedEffectOnEnd", EditConditionHides))
	int32 StackCountToRemove = INDEX_NONE;
...
private:
	...
	UE_API void InternalOnActiveGameplayEffectAdded(FActiveGameplayEffect& Effect, bool bInvokePredictedEffects);
	UE_API void InternalOnActiveGameplayEffectRemoved(FActiveGameplayEffect& Effect, bool bInvokePredictedEffects, const FGameplayEffectRemovalInfo& GameplayEffectRemovalInfo);

	UE_API void RemoveActiveGameplayEffectGrantedTagsAndModifiers(const FActiveGameplayEffect& Effect, bool bInvokePredictedEffects, bool bPredictionRejected = false);
	UE_API void AddActiveGameplayEffectGrantedTagsAndModifiers(FActiveGameplayEffect& Effect, bool bInvokePredictedEffects);
...
private:
	...
	void DeletePendingActiveGameplayEffect();
...
public:
	...
	UE_API EGameplayEffectStackingType GetStackingType() const;
