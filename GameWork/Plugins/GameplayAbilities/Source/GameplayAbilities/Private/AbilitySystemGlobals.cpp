...
void UAbilitySystemGlobals::InitGlobalData()
{
	...
	LLM_SCOPE_BYNAME(TEXT("AbilitySystem"));
	GetGlobalCurveTable();
...
void UAbilitySystemGlobals::PerformDeveloperSettingsUpgrade()
{
	...
	if (bUpgraded)
	{
		UE_LOGF(LogAbilitySystem, Warning, "AbilitySystemGlobals' Tags did not agree with GameplayAbilitiesDeveloperSettings.  Updating GameplayAbilitiesDeveloperSettings Config to use Tags from AbilitySystemGlobals");

		bool bSuccess = DeveloperSettings->TryUpdateDefaultConfigFile();
		if (!bSuccess)
		{
			UE_LOGF(LogAbilitySystem, Warning, "AbilitySystemGlobals config file (DefaultGame.ini) couldn't be saved. Make sure the file is writable to update it.");
		}
	}
	...
	for (const FSoftObjectPath& TableName : AttribDefaultTableNames)
	{
		...
		else
		{
			...
			if (AttribTable)
			{
				...
				if (!AttribTable->OnCurveTableChanged().IsBoundToObject(this))
				{
					AttribTable->OnCurveTableChanged().AddUObject(this, &ThisClass::OnAttributeDefaultTableChanged);
				}
				bModified = true;
			}
		}
				...
				if (Found->IsEmpty())
				{
					...
					if (!DeveloperSettings->GlobalAttributeSetDefaultsTableNames.Contains(TableName))
					{
						...
						if (UCurveTable* AttribTable = Cast<UCurveTable>(TableName.ResolveObject()))
						{
							...
							{
								AttribTable->OnCurveTableChanged().RemoveAll(this);
								bModified = true;
							}
						}
					}
...
void UAbilitySystemGlobals::OnAttributeDefaultTableChanged()
{
	ReloadAttributeDefaults();
}
...
void FNetSerializeScriptStructCache::InitForType(UScriptStruct* InScriptStruct)
{
	...
	for (TObjectIterator<UScriptStruct> It; It; ++It)
	{
		...
		{
			UE_LOGF(LogAbilitySystem, Verbose, "FNetSerializeScriptStructCache::InitForType: found struct type %ls",
				*It->GetName());
