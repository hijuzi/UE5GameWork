...
{
	// GetOwningAbilitySystemComponent will only return a valid pointer if the effect is still active
	return Handle.IsValid() && Handle.GetOwningAbilitySystemComponent();
}
...
bool UAbilitySystemBlueprintLibrary::IsGameplayAbilityActive(const UGameplayAbility* GameplayAbility)
{
	if (!GameplayAbility)
	{
		UE_LOGF(LogAbilitySystem, Error, "%s passed in invalid (null) GameplayAbility", __func__);
		return false;
	}
	else if (!GameplayAbility->IsInstantiated())
	{
		UE_LOGF(LogAbilitySystem, Error, "%s passed a non-instantiated instance: %ls", __func__, *GetNameSafe(GameplayAbility));
		return false;
	}
