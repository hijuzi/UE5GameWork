...
extern TAutoConsoleVariable<bool> CVarGESimOnlyTags;
...
namespace UE::GameplayEffect
{
	...
	TAutoConsoleVariable<bool> CVarFixPredictedTagApplication(
		TEXT("AbilitySystem.Fix.PredictedTagApplication"),
		true,
		TEXT("Default: True.  When enabled Gameplay Effect tag application usesthe same logic as Gameplay Cues so that application and removal works correctly for predicted effects."),
		ECVF_Default);

	TAutoConsoleVariable<bool> CVarFixPendingGameplayEffectHeadNextAtTeardown(
		TEXT("AbilitySystem.Fix.PendingGameplayEffectHeadNextAtTeardown"),
		true,
		TEXT("Default: True.  When enabled we will prevent the Pending Gameplay Effect Head from pointing to itself through Next pointer during teardown to prevent a double call to delete."),
		ECVF_Default);

	TAutoConsoleVariable<bool> CVarFixAllPendingGameplayEffectsHeadNextsAtTeardown(
		TEXT("AbilitySystem.Fix.AllPendingGameplayEffectsHeadNextsAtTeardown"),
		true,
		TEXT("Default: True.  When enabled we will prevent all the Pending Gameplay Effects Head from pointing to itself through Next pointers during teardown to prevent any double call to delete."),
		ECVF_Default);

	TAutoConsoleVariable<bool> CVarFixVerboseLogDeletionOfPendingGameplayEffectsHead(
		TEXT("AbilitySystem.Fix.VerboseLogDeletionOfPendingGameplayEffectsHead"),
		true,
		TEXT("Default: True.  When enabled we will verbose log each gameplay effect deletion."),
		ECVF_Default);
...
void UGameplayEffect::PostInitProperties()
{
	...
	if (!IsInBlueprint())
	{
		...
		{
			UE_LOGF(LogGameplayEffects, Warning, "%ls: %ls", *GetName(), *WarningText.ToString());
		}
		for (const FText& ErrorText : Errors)
		{
			UE_LOGF(LogGameplayEffects, Error, "%ls: %ls", *GetName(), *ErrorText.ToString());
		}
	}
#endif
}
...
{
	UE_LOGF(LogGameplayEffects, Warning, "%s on %ls: The implementation and method name did not match.  Use GetGrantedTags().HasTag() to check against the tags this GameplayEffect will Grant to the Actor.", __func__, *GetName());
...
{
	UE_LOGF(LogGameplayEffects, Warning, "%s on %ls: The implementation and method name did not match.  Use GetGrantedTags().HasAll() to check against the tags this GameplayEffect will Grant to the Actor.", __func__, *GetName());
...
{
	UE_LOGF(LogGameplayEffects, Warning, "%s on %ls: The implementation and method name did not match.  Use GetGrantedTags().HasAny() to check against the tags this GameplayEffect will Grant to the Actor.", __func__, *GetName());
...
void UGameplayEffect::PostCDOCompiledFixupSubobjects()
{
	...
	for (const UGameplayEffectComponent* ParentComponent : Archetype->GEComponents)
	{
		...
		if (!ChildComponent)
		{
			...
			UE_LOGF(LogGameplayEffects, Verbose, "%ls is manually duplicating Archetype %ls because they were not inherited through automatic instancing", *GetFullNameSafe(this), *GetFullNameSafe(ParentComponent));
			ChildComponent = DuplicateObject(ParentComponent, this, ParentComponentName);
		}
		else
		{
			UE_LOGF(LogGameplayEffects, Verbose, "%ls is re-adding inherited copy of Archetype %ls to components array", *GetFullNameSafe(this), *GetFullNameSafe(ParentComponent));
		}
...
void UGameplayEffect::PostCDOCompiled(const FPostCDOCompiledContext& Context)
{
	...
	UE_LOGF(LogGameplayEffects, VeryVerbose, "%ls: Running Upgrade code. Current Version = %d", *GetName(), int(GetVersion()));
...
void UGameplayEffect::OnApplied(FActiveGameplayEffectsContainer& ActiveGEContainer, FGameplayEffectSpec& GESpec, FPredictionKey& PredictionKey) const
{
	...
	UE_LOGF(LogGameplayEffects, Verbose, "Applied: %ls", *GetNameSafe(GESpec.Def));
}
...
bool FGameplayEffectModifierMagnitude::AttemptCalculateMagnitude(const FGameplayEffectSpec& InRelevantSpec, OUT float& OutCalculatedMagnitude, bool WarnIfSetByCallerFail, float DefaultSetbyCaller) const
{
	...
	if (bCanCalc)
	{
		...
		switch (MagnitudeCalculationType)
		{
			...
			default:
				UE_LOGF(LogGameplayEffects, Error, "Unknown MagnitudeCalculationType %d in AttemptCalculateMagnitude", (int32)MagnitudeCalculationType);
...
{
	return EffectClass == Other.EffectClass && RequiredSourceTags == Other.RequiredSourceTags && RemovalPolicy == Other.RemovalPolicy && StackCountToRemove == Other.StackCountToRemove;
}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	for (const FGameplayAbilitySpecDef& AbilitySpecDef : InDef->GrantedAbilities)
	{
		...
		UE_LOGF(LogGameplayEffects, Error, "%ls had GrantedAbilities dynamically added to it. This functionality is being deprecated", *GetNameSafe(InDef));
...
void FGameplayEffectSpec::CalculateModifierMagnitudes()
{
	for(int32 ModIdx = 0; ModIdx < Modifiers.Num(); ++ModIdx)
	{
		...
		if (ModDef.ModifierMagnitude.AttemptCalculateMagnitude(*this, ModSpec.EvaluatedMagnitude) == false)
		{
			...
			UE_LOGF(LogGameplayEffects, Warning, "Modifier on spec: %ls was asked to CalculateMagnitude and failed, falling back to 0.", *ToSimpleString());
		}
	}
}
...
float FGameplayEffectSpec::GetSetByCallerMagnitude(FName DataName, bool WarnIfNotFound, float DefaultIfNotFound) const
{
	...
	{
		UE_LOGF(LogGameplayEffects, Error, "FGameplayEffectSpec::GetMagnitude called for Data %ls on Def %ls when magnitude had not yet been set by caller.", *DataName.ToString(), *Def->GetName());
	}
...
float FGameplayEffectSpec::GetSetByCallerMagnitude(FGameplayTag DataTag, bool WarnIfNotFound, float DefaultIfNotFound) const
{
	...
	{
		UE_LOGF(LogGameplayEffects, Error, "FGameplayEffectSpec::GetMagnitude called for Data %ls on Def %ls when magnitude had not yet been set by caller.", *DataTag.ToString(), *Def->GetName());
	}
...
void FActiveGameplayEffect::PreReplicatedRemove(const struct FActiveGameplayEffectsContainer &InArray)
{
	...
	if (Spec.Def == nullptr)
	{
		UE_LOGF(LogGameplayEffects, Error, "Received PreReplicatedRemove with no UGameplayEffect def.");
		return;
	}
	...
	GameplayEffectRemovalInfo.OwningASC = InArray.Owner;
...
void FActiveGameplayEffect::PostReplicatedAdd(const struct FActiveGameplayEffectsContainer &InArray)
{
	...
	if (Spec.Def == nullptr)
	{
		UE_LOGF(LogGameplayEffects, Error, "FActiveGameplayEffect::PostReplicatedAdd Received ReplicatedGameplayEffect with no UGameplayEffect def. (%ls)", *Spec.GetEffectContext().ToString());
		return;
	}
	...
	if (Spec.Modifiers.Num() != Spec.Def->Modifiers.Num())
	{
		...
		UE_LOGF(LogGameplayEffects, Error, "FActiveGameplayEffect::PostReplicatedAdd: Spec.Modifiers.Num() != Spec.Def->Modifiers.Num(). Spec: %ls", *Spec.ToSimpleString());
	...
	// Do stuff for adding GEs (add mods, tags, *invoke callbacks*).  But do NOT invoke the GameplayCues or tags as we don't know if this GE ends up inhibited or not (thus the bPendingRepOnActiveGC variables).
	constexpr bool bInvokePredictedEffects = false;
	const_cast<FActiveGameplayEffectsContainer&>(InArray).InternalOnActiveGameplayEffectAdded(*this, bInvokePredictedEffects);	// Const cast is ok. It is there to prevent mutation of the GameplayEffects array, which this wont do.
...
void FActiveGameplayEffect::PostReplicatedChange(const struct FActiveGameplayEffectsContainer &InArray)
{
	...
	if (Spec.Def == nullptr)
	{
		UE_LOGF(LogGameplayEffects, Error, "FActiveGameplayEffect::PostReplicatedChange Received ReplicatedGameplayEffect with no UGameplayEffect def. (%ls)", *Spec.GetEffectContext().ToString());
		return;
	}
...
FActiveGameplayEffectsContainer::~FActiveGameplayEffectsContainer()
{
	...
	{
		if (!ensureAlwaysMsgf(!PendingGameplayEffectHead || PendingGameplayEffectHead != PendingGameplayEffectHead->PendingNext, TEXT("Pending Gameplay Effect Head is pointing to the same location as Pending Next this shouldn't happen'")))
		{
			ensure(HasActiveGameplayEffectFix(EActiveGameplayEffectFix::OmitPendingNextOnCopy));

			if (CVarFixPendingGameplayEffectHeadNextAtTeardown.GetValueOnAnyThread())
			{
				// setting next to null so that we don't call delete on the same memory twice in the code below
				PendingGameplayEffectHead->PendingNext = nullptr;
			}
		}
...
void FActiveGameplayEffectsContainer::DeletePendingActiveGameplayEffect()
{
	using namespace UE::GameplayEffect;
	...
	if (!ensureAlwaysMsgf(PendingGameplayEffectHead != PendingGameplayEffectHead->PendingNext, TEXT("Pending Gameplay Effect Head is pointing to the same location as Pending Next this shouldn't happen'")))
	{
		ensure(HasActiveGameplayEffectFix(EActiveGameplayEffectFix::OmitPendingNextOnCopy));

		if (CVarFixPendingGameplayEffectHeadNextAtTeardown.GetValueOnAnyThread())
		{
			UE_LOGF(LogGameplayEffects, Warning, "Setting PendingNext to null for pointing to self on Pending ActiveGE: %ls", *PendingGameplayEffectHead->GetDebugString());
			// setting next to null so that we don't call delete on the same memory twice in the code below
			PendingGameplayEffectHead->PendingNext = nullptr;
		}
	}

	if (CVarFixVerboseLogDeletionOfPendingGameplayEffectsHead.GetValueOnAnyThread())
	{
		UE_LOG(LogGameplayEffects, Log, TEXT("Deleting pending gameplay effect: %s. Pending next effect: %s. Owner: %s."), *PendingGameplayEffectHead->GetDebugString(), PendingGameplayEffectHead->PendingNext ? *PendingGameplayEffectHead->PendingNext->GetDebugString() : TEXT("Next is null"), Owner ? *Owner->GetFullName() : TEXT("Owner is null"));
	}
...
void FActiveGameplayEffectsContainer::PredictivelyExecuteEffectSpec(FGameplayEffectSpec& Spec, FPredictionKey PredictionKey, const bool bPredictGameplayCues /*= false*/)
{
	...
	if (bPredictGameplayCues)
	{
		...
		if (InvokeGameplayCueExecute && SpecToUse.Def->GameplayCues.Num())
		{
			...
			UE_LOGF(LogGameplayEffects, Log, "Invoking Execute GameplayCue for %ls", *SpecToUse.ToSimpleString());
...
FAggregatorRef& FActiveGameplayEffectsContainer::FindOrCreateAttributeAggregator(const FGameplayAttribute& Attribute)
{
	...
	UE_LOGF(LogGameplayEffects, Log, "Creating new entry in AttributeAggregatorMap for %ls. CurrentValue: %.2f", *Attribute.GetName(), CurrentBaseValueOfProperty);
...
void FActiveGameplayEffectsContainer::CleanupAttributeAggregator(const FGameplayAttribute& Attribute)
{
	...
	if (RefPtr)
	{
		UE_LOGF(LogGameplayEffects, Log, "Removing entry in AttributeAggregatorMap for %ls.", *Attribute.GetName());
...
void FActiveGameplayEffectsContainer::OnAttributeAggregatorDirty(FAggregator* Aggregator, FGameplayAttribute Attribute, bool bFromRecursiveCall)
{
	...
	if (Owner->IsNetSimulating())
	{
		if (FScopedAggregatorOnDirtyBatch::GlobalFromNetworkUpdate && Aggregator->NetUpdateID != FScopedAggregatorOnDirtyBatch::NetUpdateID)
		{
			...
			if (!FGameplayAttribute::IsGameplayAttributeDataProperty(Attribute.GetUProperty()))
			{
				...
				UE_LOGF(LogGameplayEffects, Log, "Reverse Evaluated %ls. FinalValue: %.2f  BaseValue: %2.f.  Setting BaseValue.  (Role: %ls)", *Attribute.GetName(), FinalValue, BaseValue, *UEnum::GetValueAsString(Owner->GetOwnerRole()));
	...
	if (EvaluationParameters.IncludePredictiveMods)
	{
		...
		UE_LOGF(LogGameplayEffects, Log, "[%ls] Aggregator Evaluated %ls. OldValue: %.2f  NewValue: %.2f", *UEnum::GetValueAsString(Owner->GetOwnerRole()), *Attribute.GetName(), OldValue, NewValue);
	}
...
void FActiveGameplayEffectsContainer::OnPredictiveGameplayEffectStackCaughtUp(FActiveGameplayEffectHandle Handle)
{
	...
	if (ActiveGE)
	{
		...
		if (DeltaStacks != 0)
		{
			...
			UE_LOGF(LogGameplayEffects, Log, "Removing local predicted stacks [%d] in %s. Last Client's stack count [%d] and Current stack count [%d]", DeltaStacks, __func__, LastServerReplicatedStackCount, CurrentStackCount);
		}
	}
}
...
void FActiveGameplayEffectsContainer::SetBaseAttributeValueFromReplication(const FGameplayAttribute& Attribute, const FGameplayAttributeData& NewValue, const FGameplayAttributeData& OldValue)
{
	...
	if (RefPtr && RefPtr->Get())
	{
		...
		if (FGameplayAttribute::IsGameplayAttributeDataProperty(Attribute.GetUProperty()))
		{
			...
			UE_LOGF(LogGameplayEffects, Log, "SetBaseAttributeValueFromReplication [%ls]: %ls rewound to state NewBaseValue: %.2f  OldCurrentValue: %.2f", OwnerIsNetAuthority ? TEXT("Authority") : TEXT("Client"), *Attribute.AttributeName, ServerBaseValue, OldEvaluatedValue);
		}
...
void FActiveGameplayEffectsContainer::GetGameplayEffectStartTimeAndDuration(FActiveGameplayEffectHandle Handle, float& EffectStartTime, float& EffectDuration) const
{
	...
	UE_LOGF(LogGameplayEffects, Warning, "GetGameplayEffectStartTimeAndDuration called with invalid Handle: %ls", *Handle.ToString());
}
...
float FActiveGameplayEffectsContainer::GetGameplayEffectMagnitude(FActiveGameplayEffectHandle Handle, FGameplayAttribute Attribute) const
{
	for (const FActiveGameplayEffect& Effect : this)
	{
		if (Effect.Handle == Handle)
		{
			...
			UE_LOGF(LogGameplayEffects, Warning, "GetGameplayEffectMagnitude with Handle: %ls did not find Attribute %ls in the modifiers.", *Handle.ToString(), *Attribute.GetName());
			return -1.f;
		}
	}

	UE_LOGF(LogGameplayEffects, Warning, "GetGameplayEffectMagnitude called with invalid Handle: %ls", *Handle.ToString());
	return -1.f;
}
...
float FActiveGameplayEffectsContainer::GetAttributeBaseValue(FGameplayAttribute Attribute) const
{
	...
	{
		UE_LOGF(LogGameplayEffects, Warning, "No Owner for FActiveGameplayEffectsContainer in GetAttributeBaseValue");
	}
...
FActiveGameplayEffect* FActiveGameplayEffectsContainer::ApplyGameplayEffectSpec(const FGameplayEffectSpec& Spec, FPredictionKey& InPredictionKey, bool& bFoundExistingStackableGE)
{
	...
	if (ExistingStackableGE)
	{
		...
		const int32 ExistingStackLimit = ExistingSpec.Def->StackLimitCount;
		...
		const bool bStackCountAtLimit = ExistingSpec.GetStackCount() == ExistingStackLimit;

		// In this path, we call HandleActiveGameplayEffectStackOverflow if we have any overflowing stacks. 
		const bool bWillOverflowStacks = ExistingStackLimit > 0 && NewStackCount > ExistingSpec.Def->StackLimitCount && HasActiveGameplayEffectOverflowBehavior(EActiveGameplayEffectOverflowBehavior::HandleOverflowingStacksAlways);
		...
		if (ExistingStackLimit > 0)
		{
			NewStackCount = FMath::Min(NewStackCount, ExistingStackLimit);
		}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (DurationBaseValue > 0.f)
	{
		...
		{
			UE_LOGF(LogGameplayEffects, Error, "ActiveGE %ls Duration was modified to %.2f. Clamping to 0.1s duration.", *AppliedActiveGE->GetDebugString(), FinalDuration);
			FinalDuration = 0.1f;
		}
		else if (SpecMaxDuration > 0.0f && FinalDuration > SpecMaxDuration)
		{
			UE_LOGF(LogGameplayEffects, Verbose, "ActiveGE %ls Duration (%.2f) exceeds MaxDuration (%.2f). Clamping to MaxDuration.", *AppliedActiveGE->GetDebugString(), FinalDuration, SpecMaxDuration);
	...
	else
	{
		...
		if (ExistingStackableGE == nullptr)
		{
			...
			if (!CVarFixPredictedTagApplication.GetValueOnAnyThread())
			{
			...
			{
				InPredictionKey.NewRejectOrCaughtUpDelegate(FPredictionKeyEvent::CreateUObject(Owner, &UAbilitySystemComponent::RemoveActiveGameplayEffect_AllowClientRemoval, AppliedActiveGE->Handle, RemoveAllStacks));
			}
		}
		else // We have an existing Stackable GE
		{
			...
			const int32 StacksAppliedWithinThisApplication = NewStackCount - StartingStackCount;
			UE_LOGF(LogGameplayEffects, Log, "This Gameplay Effect [%ls] has applied [%d] stacks when it had to stack.", *AppliedActiveGE->GetDebugString(), StacksAppliedWithinThisApplication);
		}
	}
	...
	else
	{
		...
		const bool bInvokePredictedEffects = (Spec.Def->DurationPolicy != EGameplayEffectDurationType::Instant);
		InternalOnActiveGameplayEffectAdded(*AppliedActiveGE, bInvokePredictedEffects);
	}
...
void FActiveGameplayEffectsContainer::InternalOnActiveGameplayEffectAdded(FActiveGameplayEffect& Effect, const bool bInvokePredictedEffects)
{
	...
	{
		UE_LOGF(LogGameplayEffects, Error, "FActiveGameplayEffectsContainer serialized new GameplayEffect with NULL Def!");
		return;
	}
	...
	if (IsValid(Owner))
	{
		Owner->SetActiveGameplayEffectInhibit(MoveTemp(EffectHandle), !bActive, bInvokePredictedEffects);
	}
}

void FActiveGameplayEffectsContainer::AddActiveGameplayEffectGrantedTagsAndModifiers(FActiveGameplayEffect& Effect, bool bInvokePredictedEffects)
{
	if (Effect.Spec.Def == nullptr)
	{
		UE_LOGF(LogGameplayEffects, Error, "AddActiveGameplayEffectGrantedTagsAndModifiers called with null Def!");
		return;
	}

	if (!Owner)
	{
		UE_LOGF(LogGameplayEffects, Error, "AddActiveGameplayEffectGrantedTagsAndModifiers called with null Owner!");
		return;
	}
	...
	if (CVarGESimOnlyTags.GetValueOnAnyThread())
	{
		// With Minimal Replication replicate tags to simulated proxies and the GE will manage the server and owning client.
		// Without Minimal Replication GE's are replicated fully to simulated proxies and will manage the tags on all clients.
		// Update our owner with the tags this GameplayEffect grants them
		Owner->UpdateTagMap(Effect.Spec.Def->GetGrantedTags(), 1, ShouldUseMinimalReplication() ? EGameplayTagReplicationState::SimulatedTagOnly : EGameplayTagReplicationState::None);
		Owner->UpdateTagMap(Effect.Spec.DynamicGrantedTags, 1, ShouldUseMinimalReplication() ? EGameplayTagReplicationState::SimulatedTagOnly : EGameplayTagReplicationState::None);
	}
	else
	{
		// Generic Replication will replicate Tags w/ Counts correctly.  So we should only apply the tags if we're predicting, otherwise we will get the tags of a Server-Initiated GE through GameplayTagCountContainer replication.
		// Iris Replication is more complicated. CVarReplicateTagCountContainerWithIris indicates that we do not have working Tag Count replication through GameplayTagCountContainerNetSerializer. Instead, we replicate using the legacy path of separated containers (Minimal/Loose).
		// The complex bShouldApplyTags case is we should still apply the tags of a Server-Initiated GE because we won't get those Tags through Iris GameplayTagCountContainerNetSerializer replication.  We should assume CVarFixPredictedTagApplication is true.
		...
		const bool bShouldApplyTags = UE::GameplayEffect::CVarFixPredictedTagApplication.GetValueOnAnyThread() ? (bInvokePredictedEffects || bIrisWithoutTagCountSerializer) : (!Effect.bPostPredictObject || bIrisWithoutTagCountSerializer);
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (!Owner->bSuppressGameplayCues)
	{
		for (const FGameplayEffectCue& Cue : Effect.Spec.Def->GameplayCues)
		{
			...
			// Note: This is going to ignore bInvokePredictedEffects and invoke them anyway (with a bunch of caveats e.g. you're the server and ignoring them)
...
bool FActiveGameplayEffectsContainer::RemoveActiveGameplayEffect(FActiveGameplayEffectHandle Handle, int32 StacksToRemove, bool bPredictionRejected)
{
	...
	UE_LOGF(LogGameplayEffects, Log, "RemoveActiveGameplayEffect called with invalid Handle: %ls", *Handle.ToString());
	return false;
}
...
bool FActiveGameplayEffectsContainer::InternalRemoveActiveGameplayEffect(int32 Idx, int32 StacksToRemove, bool bPrematureRemoval, bool bPredictionRejected)
{
	...
	if (ensure(Idx < GetNumGameplayEffects()))
	{
		...
		UE_LOGF(LogGameplayEffects, Verbose, "Removing: %ls. Auth: %d. NumToRemove: %d", *Effect.GetDebugString(), IsNetAuthority(), StacksToRemove);
		UE_IFVLOG(
			AActor * OwnerActor = Owner->GetOwnerActor();
		...
		GameplayEffectRemovalInfo.OwningASC = Owner;
		...
		if (IsLocked)
		{
			...
			UE_LOGF(LogGameplayEffects, Verbose, "Begin Pending Remove: %ls. Auth: %d", *Effect.GetDebugString(), IsNetAuthority());
		}
		else
		{
	...
	UE_LOGF(LogGameplayEffects, Warning, "InternalRemoveActiveGameplayEffect called with invalid index: %d", Idx);
	return false;
}
...
void FActiveGameplayEffectsContainer::InternalOnActiveGameplayEffectRemoved(FActiveGameplayEffect& Effect, bool bInvokePredictedEffects, const FGameplayEffectRemovalInfo& GameplayEffectRemovalInfo)
{
	...
	if (Effect.Spec.Def)
	{
		...
		{
			RemoveActiveGameplayEffectGrantedTagsAndModifiers(Effect, bInvokePredictedEffects, GameplayEffectRemovalInfo.bPredictionRejected);
		}
	...
	{
		UE_LOGF(LogGameplayEffects, Warning, "InternalOnActiveGameplayEffectRemoved called with no GameplayEffect: %ls", *Effect.Handle.ToString());
	}
...
void FActiveGameplayEffectsContainer::RemoveActiveGameplayEffectGrantedTagsAndModifiers(const FActiveGameplayEffect& Effect, bool bInvokePredictedEffects, bool bPredictionRejected)
{
	...
	else
	{
		...
		// The complex bShouldApplyTags case is we should still apply the tags of a Server-Initiated GE because we won't get those Tags through Iris GameplayTagCountContainerNetSerializer replication.  We should assume CVarFixPredictedTagApplication is true.
		const bool bIrisWithoutTagCountSerializer = UE::Net::ShouldUseIrisReplication() && !CVarReplicateTagCountContainerWithIris.GetValueOnAnyThread();
		const bool bShouldApplyTags = UE::GameplayEffect::CVarFixPredictedTagApplication.GetValueOnAnyThread() ? (bInvokePredictedEffects || bIrisWithoutTagCountSerializer) : (IsNetAuthority() || bPredictionRejected || bIrisWithoutTagCountSerializer);
		// Local application should be responsible for applying tags
		if (bShouldApplyTags)
		{
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
			...
			else
			{
				...
				UE_CLOGF((NumConsecutiveUnmappedReferencesDebug % HighNumberOfConsecutiveUnmappedRefs) == 0, LogAbilitySystem, Error, "%s: bHasMoreUnmappedReferences is preventing GameplayCues from firing (%u consecutive misses)", __func__, NumConsecutiveUnmappedReferencesDebug);
			}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		}
	}
...
void FActiveGameplayEffectsContainer::CheckDuration(FActiveGameplayEffectHandle Handle)
{
	...
	for (int32 ActiveGEIdx = 0; ActiveGEIdx < GameplayEffects_Internal.Num(); ++ActiveGEIdx)
	{
		...
		if (RefreshDurationTimer)
		{
			...
			{
				UE_LOGF(LogGameplayEffects, Warning, "Failed to set new timer in ::CheckDuration. Timer trying to be set for: %.2f. Removing GE instead", NewTimerDuration);
				if (!Effect.IsPendingRemove)
				{
...
void FActiveGameplayEffectsContainer::DebugCyclicAggregatorBroadcasts(FAggregator* TriggeredAggregator)
{
	for (auto It = AttributeAggregatorMap.CreateIterator(); It; ++It)
	{
		...
		if (FAggregator* Aggregator = AggregatorRef.Get())
		{
			...
			{
				UE_LOGF(LogGameplayEffects, Warning, " Attribute %ls was the triggered aggregator (%ls)", *Attribute.GetName(), *Owner->GetPathName());
			}
			else if (Aggregator->BroadcastingDirtyCount > 0)
			{
				UE_LOGF(LogGameplayEffects, Warning, " Attribute %ls is broadcasting dirty (%ls)", *Attribute.GetName(), *Owner->GetPathName());
			}
			else
			{
				continue;
			...
			for (FActiveGameplayEffectHandle Handle : Aggregator->Dependents)
			{
				...
				if (ASC)
				{
					UE_LOGF(LogGameplayEffects, Warning, "  Dependant (%ls) GE: %ls", *ASC->GetPathName(), *GetNameSafe(ASC->GetGameplayEffectDefForHandle(Handle)));
				}
			}
		}
	}
...
void FActiveGameplayEffectsContainer::DecrementLock()
{
	if (--ScopedLockCount == 0)
	{
		...
		for (int32 idx=GameplayEffects_Internal.Num()-1; idx >= 0 && PendingRemoves > 0; --idx)
		{
			...
			if (Effect.IsPendingRemove)
			{
				UE_LOGF(LogGameplayEffects, Verbose, "%ls: Finish PendingRemove: %ls. Auth: %d", *GetNameSafe(Owner->GetOwnerActor()), *Effect.GetDebugString(), IsNetAuthority());
		...
		{
			UE_LOGF(LogGameplayEffects, Error, "~FScopedActiveGameplayEffectLock has %d pending removes after a scope lock removal", PendingRemoves);
			PendingRemoves = 0;
		}
