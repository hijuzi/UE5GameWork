...
#if UE_WITH_REMOTE_OBJECT_HANDLE
	if (const FUObjectMigrationContext* MigrationContext = FUObjectMigrationContext::GetCurrentMigrationContext())
	{
		// Do not DestroyActiveState when migrating-out.  There's too many side effects which we should instead handle in PostMigrate.
...
void UAbilitySystemComponent::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);

	if (Ar.IsMigratingRemoteObjects())
	{
		bool bHasAbilityActorInfo = AbilityActorInfo.IsValid();
		Ar << bHasAbilityActorInfo;
		if (bHasAbilityActorInfo)
		{
			// Can happen when loading (the sending side had an ability info we didn't yet allocate)
			if (!AbilityActorInfo.IsValid())
			{
				AbilityActorInfo = TSharedPtr<FGameplayAbilityActorInfo>( UAbilitySystemGlobals::Get().AllocAbilityActorInfo() );
			}

			// TODO: This just uses the default serializer but does not account for polymorphic structs
			ensureMsgf(sizeof(*AbilityActorInfo.Get()) == sizeof(FGameplayAbilityActorInfo), TEXT("We do not handle polymorphic AbilityActorInfo properly"));
			FGameplayAbilityActorInfo DefaultStruct;
			FGameplayAbilityActorInfo::StaticStruct()->SerializeItem(Ar, AbilityActorInfo.Get(), &DefaultStruct);
		}

		if (Ar.IsSaving())
		{
			auto SaveTagToGASpecMap = [&Ar](TMap<FGameplayTag, TArray<FGameplayAbilitySpecHandle>>& Map)
			{
				int32 NumEntries = Map.Num();
				Ar << NumEntries;
				for (TPair<FGameplayTag, TArray<FGameplayAbilitySpecHandle>>& Pair : Map)
				{
					FGameplayTag& GameplayTag = Pair.Key;
					TArray<FGameplayAbilitySpecHandle>& GASpecs = Pair.Value;
			...
			SaveTagToGASpecMap(GameplayEventTriggeredAbilities);
			SaveTagToGASpecMap(OwnedTagTriggeredAbilities);
		}
		else if (Ar.IsLoading())
		{
			auto LoadTagToGASpecMap = [&Ar](TMap<FGameplayTag, TArray<FGameplayAbilitySpecHandle>>& Map)
			{
				int32 NumEntries = Map.Num();
				Ar << NumEntries;
				...
				TArray<FGameplayAbilitySpecHandle> GASpecs;

				for (int Idx = 0; Idx < NumEntries; ++Idx)
				{
					Ar << GameplayTag;
					Ar << GASpecs;
					Map.Emplace(GameplayTag, GASpecs);
				}
			};

			LoadTagToGASpecMap(GameplayEventTriggeredAbilities);
			LoadTagToGASpecMap(OwnedTagTriggeredAbilities);
		}
	}
}
...
void UAbilitySystemComponent::ReadyForReplication()
{
	...
	{
		for (UGameplayAbility* ReplicatedAbility : AllReplicatedInstancedAbilities)
		{
...
FActiveGameplayEffectHandle UAbilitySystemComponent::SetActiveGameplayEffectInhibit(FActiveGameplayEffectHandle&& ActiveGEHandle, bool bInhibit, bool bInvokePredictedEffects)
{
...
void UAbilitySystemComponent::UpdateTagMapSingle_Internal(const FGameplayTag& Tag, const int32 CountDelta, EGameplayTagReplicationState TagRepState)
{
	if (UE::Net::ShouldUseIrisReplication() && !CVarReplicateTagCountContainerWithIris.GetValueOnAnyThread())
	{
		...
		if (NetMode == NM_DedicatedServer || NetMode == NM_ListenServer)
		{
			...
			else if (TagRepState > EGameplayTagReplicationState::None)
			{
			...
			if (GameplayTagCountContainer.UpdateTagCount_DeferredParentRemoval(Tag, CountDelta, DeferredTagChangeDelegates))
			{
				...
				if (UE::Net::ShouldUseIrisReplication() && !CVarReplicateTagCountContainerWithIris.GetValueOnAnyThread())
				{
					...
					if (NetMode == NM_DedicatedServer || NetMode == NM_ListenServer)
					{
						...
						if (MTagMap.TagMap.Contains(Tag))
						{
							MTagMap.SetTagCount(Tag, 0);
						}

						FMinimalReplicationTagCountMap& RLTagMap = GetReplicatedLooseTags_Mutable();
						if (RLTagMap.TagMap.Contains(Tag))
						{
							RLTagMap.SetTagCount(Tag, 0);
						}
					}
				}
			}
...
bool UAbilitySystemComponent::RemoveActiveGameplayEffect(FActiveGameplayEffectHandle Handle, int32 StacksToRemove)
{
	...
	if (!EnumHasAnyFlags(static_cast<EAllowPredictiveGEFlags>(CVarAllowPredictiveGEFlagsValue), EAllowPredictiveGEFlags::AllowNativeRemoveByHandle))
	{
		if (!IsOwnerActorAuthoritative())
		{
			...
			UE_LOGF(LogAbilitySystem, Warning, "%s called without Authority when attempting to remove %ls. Fix-up code, or temporarily patch using AbilitySystem.Fix.AllowPredictiveGEFlags", __func__, *GetNameSafe(GEDef));
			return false;
		}
	}
...
float UAbilitySystemComponent::GetFilteredAttributeValue(const FGameplayAttribute& Attribute, const FGameplayTagRequirements& SourceTags, const FGameplayTagContainer& TargetTags, const TArray<FActiveGameplayEffectHandle>& HandlesToIgnore)
{
	...
	UE_CLOGF(!SourceTags.TagQuery.IsEmpty(), LogAbilitySystem, Error, "GetFilteredAttributeValue: SourceTags cannot contain a TagQuery (it is ignored)");
	UE_CLOGF(!SourceTags.IgnoreTags.IsEmpty(), LogAbilitySystem, Error, "GetFilteredAttributeValue: SourceTags cannot contain IgnoreTags (they are not used)");
	...
	else
	{
		...
		{
			UE_LOGF(LogAbilitySystemComponent, Warning, "Failed to calculate Attribute %ls. On: %ls", *Attribute.GetName(), *GetFullName());
		}
	}
...
{
	if (AllReplicatedInstancedAbilities.Find(GameplayAbility) == INDEX_NONE)
	{
		AllReplicatedInstancedAbilities.Add(GameplayAbility);
...
{
	const bool bWasRemoved = AllReplicatedInstancedAbilities.RemoveSingle(GameplayAbility) > 0;
...
void UAbilitySystemComponent::RemoveAllReplicatedInstancedAbilities()
{
	...
	AllReplicatedInstancedAbilities.Empty();
}
