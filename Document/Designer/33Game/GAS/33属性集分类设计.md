...
`Gradient` 与所有其它集的分歧在挂载对象：它是队伍级的，一队一份。并进任何一个角色集，就会变成每角色一份，既不符合官方的共享分段条，复制时也会分叉。
...
| 属性 | 说明 | 复制 |
|---|---|---|
| `Level` | 当前等级，范围 `[1, MaxLevel]` | 是 |
| `MaxLevel` | 等级上限，默认 99 | 是 |
| `Experience` | 当前等级内已积累的经验 `[0, 下一级所需)` | 是 |
| `ExperienceToNextLevel` | 升到下一级所需经验 | 是 |
| `AttributePoints` | 未分配加点，每级发 3 点 | 是 |
| `SkillPoints` | 未分配技能点，每级发 1 点 | 是 |
| `ExperienceGain` | 元属性，经验入账，结算后清零 | 否 |
...
| 元属性 | `Damage`、`Healing`、`ShieldBreak`，不复制，结算后清零 | `ResolveDamage()`、`ResolveHealing()`、`ResolveShieldBreak()` |
...
倒地标记和护盾层数都放在 `PostAttributeChange` 里派生，而不是 `PostGameplayEffectExecute`。因为客户端收到复制值后重新聚合也会走这条回调，放这里服务器和客户端的状态天然一致，不用额外复制一个标记。
...
所有类都遵循同一套写法：`FGameplayAttributeData` + `ATTRIBUTE_ACCESSORS_BASIC` + `DOREPLIFETIME_CONDITION_NOTIFY(..., REPNOTIFY_Always)` + `GAMEPLAYATTRIBUTE_REPNOTIFY`。
